...
![Plataforma](https://img.shields.io/badge/Plataforma-Azure%20%7C%20Windows%20Server%202022-blue)
...
|---|---|---|
| Branyel Estifenso Pérez Díaz | 2024-1489 | Perito Forense Digital |
| Francis Antonio Vidal Acosta | 2024-1183 | Perito Forense Digital |
| Elian Felix Leonardo | 2023-0961 | Perito Forense Digital |
...
> El laboratorio fue montado íntegramente en **Microsoft Azure (East US)**, simulando un entorno corporativo real con Active Directory.

| Componente | Especificación |
|---|---|
| **Servidor víctima** | Windows Server 2022 — Domain Controller (`SRV-DC01`) |
| **Dominio AD** | `pathsecure.local` |
| **IP del servidor** | `10.0.0.4` (Azure East US) |
| **Estación atacante** | Windows 10 — Azure East US |
...
23/04/2026  4:15 AM  ──  El troyano se activa tras reinicio y establece beacon C2 → puerto 1177/TCP
...
23/04/2026  4:50 AM  ──  Atacante escala privilegios al Domain Controller (pathsecure.local)
...
> Captura del contenido volátil (RAM de 94 GB) para preservar procesos activos en el momento del análisis.
...
> FTK Imager verificó la integridad de la imagen mediante MD5 y SHA1. Resultado: **MATCH en ambos algoritmos**, confirmando que la imagen es copia fiel del original sin alteraciones.
...
Autoruns identificó la **tarea programada maliciosa** `WindowsUpdateHelper` configurada para ejecutar `PowerShell.exe` con `ExecutionPolicy Bypass` sobre el script `svchost_helper.ps1`, confirmando el mecanismo de persistencia y detonación de la bomba lógica.
...
El Visor de Eventos proporcionó la **evidencia directa e irrefutable de la detonación**. Los logs de seguridad registraron el **Event ID 4724** (An attempt was made to reset an account's password) para cada usuario del dominio, todos con timestamp idéntico: **5:13:03 AM**.
...
> Documento completo disponible en [`docs/Cadena_de_Custodia.pdf`](docs/Cadena_de_Custodia.pdf)

| Objeto | Descripción | Fecha/Hora de Recolección | Entregado por | Recibido por |
|---|---|---|---|---|
| Volcado RAM | 84 GB DDR4 4800 MHz DIMM | 23/04/2026 — 2:58 PM | Elian Felix L. (2023-0961) | Francis Antonio (2024-1183) |
| Disco Duro | WD10EZEX 1TB — Imagen E01 adquirida con FTK Imager | 23/04/2026 — 2:58 PM | Branyel Pérez (2024-1489) | Francis Antonio (2024-1183) |

**Oficiales:** Elian Felix Leonardo / Branyel Pérez / Francis Antonio Vidal  
**Caso:** 2026-001 · **Delito:** Acceso ilícito a la red de la empresa  
**Víctima:** Path Secure SRL · **Lugar:** Oficinas, Santo Domingo Este

---

## Conclusiones

La investigación forense confirma que el incidente del 23 de abril de 2026 fue un **ataque dirigido y premeditado** que siguió una cadena de compromiso clásica:

1. **Phishing como vector inicial** — El atacante empleó ingeniería social con un correo de alta credibilidad que explotó la confianza en comunicaciones internas de RRHH.

2. **RAT para acceso persistente** — NJRat v0.7d proporcionó al atacante acceso remoto total: keylogger, file manager y shell remoto sobre el equipo comprometido.

3. **Error humano crítico** — El almacenamiento de credenciales administrativas en texto plano (`passwords_temporal.txt`) en el escritorio del servidor fue el factor decisivo que permitió la escalada al Domain Controller. Constituye una violación grave de políticas de seguridad.

4. **Bomba lógica efectiva** — La combinación de un script PowerShell camuflado como proceso del sistema y una Scheduled Task con nombre legítimo resultó en una ejecución exitosa sin detección previa.

5. **Impacto máximo calculado** — La detonación a las 5:13 AM garantizó la interrupción máxima al coincidir con el inicio de la jornada laboral.
...
- **Prohibir credenciales en texto plano** — implementar un gestor de contraseñas corporativo (ej. CyberArk, HashiCorp Vault)
- **Capacitar empleados** en identificación de correos phishing y spear phishing
- **Restringir ejecución de scripts PowerShell** no firmados mediante Group Policy (Constrained Language Mode)
...
- **Segmentar la red** para limitar el movimiento lateral ante el compromiso de una estación de trabajo
...
|---|---|
| [Informe de Investigación Forense](docs/Informe_Investigacion_Forense_PathSecure.pdf) | Informe técnico completo con análisis detallado de cada herramienta y hallazgo |
| [Informe Ejecutivo](docs/Informe_Ejecutivo.pdf) | Resumen ejecutivo del incidente para dirección y partes no técnicas |
| [Cadena de Custodia](docs/Cadena_de_Custodia.pdf) | Formulario oficial de cadena de custodia de las evidencias digitales |
